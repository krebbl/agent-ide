...
export default function AddProjectDialog({ onClose }: AddProjectDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="flex w-[560px] max-h-[80vh] flex-col rounded-lg border border-[var(--color-surface0)] bg-[var(--color-mantle)] shadow-2xl">
        <div className="flex items-center justify-between border-b border-[var(--color-surface0)] px-4 py-3">
          <h2 className="text-sm font-semibold text-[var(--color-text)]">Add Project</h2>
          <button onClick={onClose} className="text-[var(--color-overlay1)] hover:text-[var(--color-text)]">
            <X size={18} />
          </button>
        </div>

        <div className="flex border-b border-[var(--color-surface0)]">
          <button
